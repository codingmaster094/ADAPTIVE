...
export default function WhatWeDo() {
  ...
  return (
    <>
      <PageLoader />
      <Head>
        <title>{retail_investorsData?.seo_meta.meta_title.replace(/&amp;/g, '&')}</title>
        <meta property="og:title" content={retail_investorsData?.seo_meta.meta_title.replace(/&amp;/g, '&')} />
        <meta property="og:site_name" content="Adaptive Investments" />
        <meta property="og:description" content={retail_investorsData?.seo_meta.meta_description.replace(/&amp;/g, '&')} />
      ...
      {retail_investorsData?.retail_investors_banner_image && (
        <Image 
          ...
          width={630} 
          height={400} 
          alt={retail_investorsData?.retail_investors_banner_main_title || "Banner image"} // Added alt prop
        />
            )}
      ...
      <div className="sectiondivide vcolumn gap100">
        <div className="featureview context">
          <div className="sectiontitle">
          {
            ...
            <p className="subhighlight">{retail_investorsData?.retail_investors_what_we_do_sub_title}</p>
          }
            <h2>{retail_investorsData?.retail_investors_what_we_do_main_title}</h2>
          </div>
          <div className="productcardsec">
            {retail_investorsData?.retail_investors_what_we_do_all_tools?.map((val, i) => (
              <Productcard
                ...
                link=""
              >
                <p dangerouslySetInnerHTML={{ __html: val.retail_investors_what_we_do_all_tools_content?.replace(/<p>/g, '').replace(/<\/p>/g, '') }} />
              </Productcard>
            ))}
          </div>
        </div>
        <div className="featureview context">
          <div className="sectiontitle">
          {
            ...
            <p className="subhighlight">{retail_investorsData?.retail_investors_changes_main_title}</p>
          }
            <h2>{retail_investorsData?.retail_investors_changes_sub_title}</h2>
          </div>
          <div className="productcardsec">
            {retail_investorsData?.retail_investors_what_we_do_all_tools?.map((val, i) => (
              <Productcard
      ...
      <ShieldCard
        ...
        normaltext={retail_investorsData?.retail_investors_downturns_content?.replace(/<p>/g, '').replace(/<\/p>/g, '').replace(/&amp;/g, '&')}
      >
      ...
      <div className="sectiondivide vcolumn gap100">
        <div className="featureview context">
          <div className="sectiontitle">
            <h2>{retail_investorsData?.retail_investors_benefits_main_title}</h2>
            <p className="text-center" dangerouslySetInnerHTML={{ __html: retail_investorsData?.retail_investors_benefits_content?.replace(/<p>/g, '').replace(/<\/p>/g, '') }}></p>
          </div>
          <div className="feturecardcon">
            {retail_investorsData?.retail_investors_all_benefits?.map((val, i) => (
              <FeatureCard 
                key={i} // Added key prop
                ...
                content={val?.retail_investors_all_benefits_content?.replace(/<p>/g, '').replace(/<\/p>/g, '')}
                hreflink="javascript:;"
              />
            ))}
          </div>
